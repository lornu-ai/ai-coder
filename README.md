...
⚠️ **SECURITY WARNING**: Agent mode executes bash commands generated by the LLM. This carries inherent risks from prompt injection attacks. **Only use agent mode with trusted prompts and in safe environments.** Never use `--yes` flag in production or with untrusted inputs.
...
# Auto-approve mode - executes without confirmation (⚠️ use with extreme caution)
...
- ✅ **Always use interactive mode** (without `--yes`) to review commands before execution
- ✅ **Use agent mode only in safe, isolated environments** (VMs, containers, throwaway systems)
- ✅ **Verify all commands** before confirming execution
- ✅ **Limit permissions**: Run ai-coder with minimal necessary permissions
- ✅ **Use specific prompts**: Be explicit about what you want (avoids ambiguity)
- ❌ **Never use `--yes` flag** with untrusted or adversarial prompts
- ❌ **Never use agent mode in production systems** or critical environments
- ❌ **Don't pipe untrusted input** to ai-coder
